...
foreach ([$lib, $legacy, $plugin, $package] as $manifest) {
...
$feed = simplexml_load_file($argv[7]);
...
if (simplexml_load_file($argv[8]) === false) {
    fwrite(STDERR, "Invalid Core changelog XML.\n"); exit(1);
}
' "$ROOT/VERSION" "$LIB_SRC/xdecarocore.xml" "$LEGACY_MANIFEST" "$PLUGIN_SRC/xdecarocore.xml" "$PACKAGE_SRC/pkg_xdecarocore.xml" "$ASSET_REGISTRY" "$UPDATE_FEED" "$CHANGELOG_XML"
...
artifacts = [library, legacy, plugin, package]
...
with zipfile.ZipFile(legacy) as archive:
    ...
    if archive.read("src/Integration/EntityReference.php") != zipfile.ZipFile(library).read("src/Integration/EntityReference.php"):
...
with zipfile.ZipFile(package) as archive:
    expected = {"pkg_xdecarocore.xml", "lib_xdecarocore.zip", "lib_xdecarocorelegacy.zip", "plg_system_xdecarocore.zip"}
