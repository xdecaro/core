...
if (!is_string($text) || $count < 1) {
    fwrite(STDERR, "Unable to update Core SHA-256 in update feed.\n");
    exit(1);
}
...
if ($xml === false) {
    fwrite(STDERR, "Updated Core feed is not valid XML.\n");
    exit(1);
}

foreach ($xml->update as $update) {
    if (trim((string) $update->sha256) !== $digest) {
        fwrite(STDERR, "Core feed checksum verification failed.\n");
        exit(1);
    }
}
